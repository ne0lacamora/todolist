...
<div class="col-sm-4 mt-3">
    <div class="card border-success">
        <div class="card-header bg-success">
            <h4 class="text-white">{{ mysingle.nombre }}</h4>
        </div>
        <div class="bg-white card-body">
            ...
            <p class="font-weight-bold" v-else>{{ mysingle.nombre }}</p>
            <!-- Descripción -->
            <textarea v-if="editMode" cols="30" rows="5" class="mt-2 form-control" v-model="mysingle.descripcion"></textarea>
            <p v-else class="font-weight-light">{{ mysingle.descripcion }}</p>
            <div class="clearfix">
                <hr>
            </div>
            <!-- Estados -->
            <p class="mt-1 font-weight-bold">Estado de la Tarea</p>
            <p class="text-success text-uppercase font-weight-bold" v-if="mysingle.estado === 'activa'">{{ mysingle.estado }}</p>
            <p class="text-warning text-uppercase font-weight-bold" v-else-if="mysingle.estado === 'pendiente'">{{ mysingle.estado }}</p>
            <p class="text-danger text-uppercase font-weight-bold" v-else>{{ mysingle.estado }}</p>
